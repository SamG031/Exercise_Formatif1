...
void InscrireUnEleve(string inCode, string inNom, string inPrenom)
{
	...
	lesEleve[nombreElevesReels].inscrireEleve(codeE, nomE, prenomE);
...
int RechercherEleve(string inCodeVoulu)
{
	...
	{
		if (lesEleve[cptEleve].getCodeEleve() == inCodeVoulu)
		{
			trouve = true;
		}
		cptEleve++;
...
int RechercherActivite(string inCodeActiviteVoulue)
{
	...
	{
		if (lesActivites[cptActivite].getCode() == inCodeActiviteVoulue)
		{
			trouve = true;
		}
...
void AfficherLesInscriptions()
{
	...
	if (RechercherActivite(activite) != -1)
	{
		...
		{
			if (lesEleve[i].getActivite() != NULL)
			{
				if (lesEleve[i].getActivite()->getCode() == activite)
				{
					Afficher(lesEleve[i]);
				}
			}
